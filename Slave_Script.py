...
Poll_GPSDO = IntVar() 
# PPS Error Variables
PPS_Error_Length = 60 * 2 # length of PPS Error Arrays graph time axis (default 2 mins)
...
class NetworkConnection:
   def __init__(self):
      ...
      self.LocalAddr = "192.168.1.201"
      self.RaspN0Addr = "192.168.1.200"
      self.RaspN2Addr = "192.168.1.202"
      self.Port = 25001
      self.Buffer_Size = 1024
      #create socket
      self.clientLive = False
   ...
   def startClient(self):
      try:
        ...
        self.tcpSocket.connect((self.RaspN0Addr,self.Port))
        self.socketListener = threading.Thread(target=self.SocketListener)
        #self.socketListener.do_run = False
        self.socketListener.start()
        self.clientLive = True
        NetworkTextBox.insert(END, "Connected to Master ... \n")
        self.socketListener.do_run = True
      except Exception, e:
        print str(e)
        NetworkTextBox.insert(END, "Could not connect to Master ... \n")
        
   def stopClient(self):
    if self.clientLive == True:
      self.socketListener.do_run = False
      self.tcpSocket.close()
      self.clientLive = False
      NetworkTextBox.insert(END, "Disconnected from Master ... \n")
      
   
   def broadcastMessage(self,message):
      result = self.tcpSocket.sendall(message.encode());
      print "Sending Message", result
        
   def SocketListener(self):
      self.socketListener = threading.currentThread()
      while (getattr(self.socketListener, "do_run", True)):
        try:
           message = self.tcpSocket.recv(self.Buffer_Size).strip()
           message = str(message.decode())
           print "Received data: " , message
           if message == "":
             self.stopClient()
             NetworkTextBox.insert(END, "Master Disconnected ...\n")
           elif (message.startswith("TR") == True):
             NetworkTextBox.insert(END, "Trigger Received \n")
             Trigger.decodeTrigger(message)
        except Exception,e:
           print str(e)
        time.sleep(1)
...
class SpecGPSDO:
    def __init__(self):
      ...
      self.GpsDateTime = 0
      self.epochGpsDateTime = 1623405890.0-1
...
class Trigger:
    ...
    def setTriggerPending(self):
      ...
      if self.Delay_Trigger_Sec < 10: #don't accept a trigger deadline less than 10s away.
    ...
    def decodeTrigger(self,trigger_Message):
      trigger_Message = trigger_Message.partition("_")[2]
      self.epochGpsTriggerTime  = float(trigger_Message.partition("_")[0])
      self.triggId = int(trigger_Message.partition("_")[2])
      print "Epoch Trigger Deadline", self.epochGpsTriggerTime 
      self.triggerSet()
      self.calculatePulseDelay()
    ...
    def sendTrigger(self):
      ...
      try:
          ...
          if ((GPSDO.epochGpsDateTime - self.epochGpsTriggerTime) == 0):
            ...
            TextBox.insert(END, 'N1 Trigger Valid \n\n')
            triggValidity = True
          else:
            TextBox.insert(END, 'N1 Trigger Error of ' + str(int(GPSDO.epochGpsDateTime - self.epochGpsTriggerTime)) + ' s \n\n')
            triggValidity = False
          if  triggValidity: 
            TCPServer.broadcastMessage("N1 Trigger Valid")
          else:
            TCPServer.broadcastMessage("N1 Trigger Error")
...
mButtonListen = Button(network_control_frame,text="Connect",command=connectToMaster).grid(row=2,column=0,pady=5,padx=5)
mButtonDisconnect = Button(network_control_frame,text="Disconnect",command=disconnectFromMaster).grid(row=2,column=1,pady=5,padx=5)
